...
UA_PubSubChannelEthernet_open(const UA_PubSubConnectionConfig *connectionConfig) {
   ...
   // struct sockaddr_ll sll = { 0 };
...
#ifdef UA_ENABLE_PUBSUB_CUSTOM_PUBLISH_HANDLING
    static int use_socket_transmission_time = 1;
    ...
    if (use_socket_transmission_time && setsockopt(sockFd, SOL_SOCKET, SOCKET_TRANSMISSION_TIME, &sk_txtime, sizeof(sk_txtime))) {
