...
//Uncomment this define to enable alarm before server bootup
//#define ENABLE_ALARM_BEFORE_BOOTUP
...
addCondition_1(UA_Server *server) {
    ...
    //Create a condition source object
    ...
    if(retval != UA_STATUSCODE_GOOD) {
        ...
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                     "creation of condition source object is successful with Statuscode %s",
                     UA_StatusCode_name(retval));
    }
    //Create condition 1 of type OffNormalAlarmType
    ...
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                     "creation of condition-1 as a component of condition source object is successful with Statuscode %s",
                     UA_StatusCode_name(retval));
    return retval;
}
...
enteringConfirmedStateCallback(UA_Server *server, const UA_NodeId *condition) {
    ...
    UA_Boolean FalseInstance = false;
    UA_Variant value;
    UA_QualifiedName activeStateField = UA_QUALIFIEDNAME(0,"ActiveState");
    UA_QualifiedName activeStateIdField = UA_QUALIFIEDNAME(0,"Id");
    UA_QualifiedName retainField = UA_QUALIFIEDNAME(0,"Retain");

    UA_Variant_setScalar(&value, &FalseInstance, &UA_TYPES[UA_TYPES_BOOLEAN]);
    UA_StatusCode retval =
        UA_Server_setConditionVariableFieldProperty(server, *condition,
                                                    &value, activeStateField,
                                                    activeStateIdField);
    ...
    if(retval != UA_STATUSCODE_GOOD) {
        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                     "Setting false to retain flag failed. StatusCode %s",
                     UA_StatusCode_name(retval));
    }
...
setUpEnvironment(UA_Server *server) {
    ...
    //Define usercallback for enable state and set retain flag value to true 
    //in enable state callback
    ...
    if(retval != UA_STATUSCODE_GOOD) {
        ...
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                     "adding entering enabled state callback is successful with StatusCode %s",
                     UA_StatusCode_name(retval));
    }

    //Define usercallback for acknowlegded state and set activeid to false to deactivate the alarm
    ...
    if(retval != UA_STATUSCODE_GOOD) {
        ...
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                     "adding entering acked state callback is successful with StatusCode %s",
                     UA_StatusCode_name(retval));
    }

    //Define usercallback for confimed state and set activeid and retain flag to false to
    //deactivate and disable the alarm notification
    ...
    if(retval != UA_STATUSCODE_GOOD) {
        ...
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                     "adding entering confirmed state callback is successful with StatusCode %s",
                     UA_StatusCode_name(retval));
    }

    UA_NodeId variable_1;
    /* Add  a variable to trigger condition events */
    ...
    if(retval != UA_STATUSCODE_GOOD) {
        ...
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                     "Setting variable 1 Callback successful with statusCode %s",
                     UA_StatusCode_name(retval));
    }
    return retval;
}
...
int main (void) {
    ...
    //Setup Alarms and condition environment
    UA_StatusCode retval = setUpEnvironment(server);

#ifdef ENABLE_ALARM_BEFORE_BOOTUP
