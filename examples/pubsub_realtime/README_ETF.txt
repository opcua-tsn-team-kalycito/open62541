...
NOTE: It is always recommended to run TSN_XDP_mps_loopback application first to avoid missing the initial data published by TSN_XDP_mps_publisher
      The CSV files will be generated in the build directory containing the timestamp and counter values for benchmarking only after terminating both the applications using “Ctrl + C”
====================================================================================================================================================================
NOTE:
...
1. To test ethernet connection creation and ethernet publish unit tests(without realtime)
        cd open62541/build
        cmake -DUA_ENABLE_PUBSUB=ON -DUA_ENABLE_PUBSUB_ETH_UADP=ON -DUA_ENABLE_UNIT_TESTS=ON ..
        make
...
2. To test ethernet connection creation and ethernet publish unit tests(with realtime)
        cd open62541/build
        cmake -DUA_ENABLE_PUBSUB=ON -DUA_ENABLE_PUBSUB_ETH_UADP=ON -DUA_ENABLE_PUBSUB_ETH_UADP_ETF=ON -DUA_ENABLE_UNIT_TESTS=ON ..
        make
        The following binaries are generated in build/bin/tests folder
